...
class UNetGenerator(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_features=64, depth=4):
        ...
        for i in range(depth):
            ...
            if i == 0:
                block = nn.Sequential(nn.Conv2d(prev_ch, out_ch, 4, 2, 1), nn.LeakyReLU(0.2))
            else:
...
def train(
    ...
    adv_weight=1.0, l1_weight=50.0, fm_weight=0.0,
    ...
    # feature matching: use intermediate D features (simple implementation)
    use_fm = fm_weight > 0.0
    def feature_matching_loss(real_feats, fake_feats):
        loss = 0.0
        for rf, ff in zip(real_feats, fake_feats):
            loss = loss + nn.functional.l1_loss(ff, rf.detach())
        return loss
    ...
    mlflow.log_params({
        ...
        "adv_weight": adv_weight, "l1_weight": l1_weight, "fm_weight": fm_weight,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (xin, yin) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}")):
            ...
            with torch.cuda.amp.autocast(enabled=(device=="cuda")):
                ...
                g_fm  = 0.0
                # optionally compute feature matching if enabled (requires D to expose intermediate features; skipped here)
                g_total = adv_weight * g_adv + l1_weight * g_l1 + fm_weight * g_fm
